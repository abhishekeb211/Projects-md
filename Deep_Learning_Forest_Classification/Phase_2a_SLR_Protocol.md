...
|-----------|-------------|
| **Primary** | Identify state-of-the-art methods for HSI-LiDAR fusion in forest species classification and structural parameter extraction |
...
| **Technical** | Identify best practices for structural parameter extraction from remote sensing |
...
| **Forest Survey of India** | Government | Indian forest inventory | Secondary |
...
```
("LiDAR" OR "laser scanning" OR "ALS" OR "TLS" OR "UAV LiDAR")
AND 
("forest structure" OR "canopy height" OR "tree detection" OR "individual tree" OR "crown delineation")
AND 
("classification" OR "segmentation" OR "deep learning" OR "parameter extraction")
```
...
```
("India" OR "Indian" OR "Meghalaya" OR "Northeast India" OR "Western Ghats" OR "Himalaya")
AND 
...
**S5: ISRO Missions**
```
("HySIS" OR "AVIRIS-NG" OR "Resourcesat" OR "RISAT" OR "Cartosat")
AND 
("vegetation" OR "forest" OR "biodiversity" OR "land cover")
```

**S6: Structural Parameter Extraction**
```
("forest structure" OR "canopy height" OR "tree height" OR "crown area" OR "biomass")
AND
("remote sensing" OR "LiDAR" OR "UAV")
AND
("extraction" OR "estimation" OR "mapping" OR "deep learning")
```
...
| Classification | mapping, identification, discrimination, recognition, detection |
| Fusion | integration, combination, multi-modal, multi-sensor, synergy |
| Structure | architecture, canopy, height, crown, biomass |
...
| I2 | Published 2015-2026 | Relevance; DL era; recent methods |
...
| E4 | No classification/mapping/parameter extraction component | Methodological focus |
| E5 | Simulation-only studies (no real data) | Empirical requirement |
...
**Key Topics**: 3D-CNN, spectral attention, spatial-spectral networks, SpectralFormer, HybridSN
...
**Key Topics**: Early/mid/late fusion, feature-level integration, attention-based fusion, decision fusion
...
**Key Topics**: Spectral libraries, species separability, tropical forests, benchmark datasets
**Expected Papers**: 15-20
...
### Cluster 6: Forest Structural Parameter Extraction
**Focus**: Methods for extracting height, biomass, crown parameters
**Key Topics**: Allometry, individual tree detection, area-based approaches, machine learning
**Expected Papers**: 10-15

### Cluster 7: ISRO & DOS Missions for Vegetation Analysis (**MANDATORY**)
...
|----------|---------------|---------------|
| Web of Science | S1, S2, S3, S4, S6 | 250-350 |
| Scopus | S1, S2, S3, S4, S6 | 300-400 |
| IEEE Xplore | S1, S2, S3 | 100-150 |
| ISPRS Archives | S1, S2, S3, S4 | 60-100 |

**Estimated Initial Pool**: 700-1000 papers (with ~40% overlap)
**After Deduplication**: ~450-600 unique papers

### Phase 2: Title/Abstract Screening (Week 2-3)

| Step | Action | Expected Retention |
|------|--------|-------------------|
| 2.1 | Apply inclusion criteria (title) | 55% |
| 2.2 | Apply inclusion criteria (abstract) | 50% |
| 2.3 | Apply exclusion criteria | 70% |

**Expected After Screening**: 100-150 papers
...
| 3.2 | Quality assessment scoring | 70% |
| 3.3 | Final relevance check | 85% |

**Expected Final Corpus**: 60-100 papers

### Phase 4: Cluster Distribution

| Cluster | Target Papers | Priority |
|---------|--------------|----------|
| C1: DL for HSI | 15-18 | High |
| C2: LiDAR Forest | 10-15 | High |
| C3: HSI-LiDAR Fusion | 10-12 | Critical |
| C4: Tree Species | 12-15 | High |
| C5: UAV RS | 10-12 | Medium |
| C6: Structural Parameters | 8-12 | High |
| C7: ISRO Missions | 10-12 | **Mandatory** |
...
**Expected Additional Papers**: 15-25

### Phase 6: ISRO-Specific Collection (Parallel)

| Source | Method | Target |
|--------|--------|--------|
| ISRO website publications | Manual search | 5-10 |
| SAC Annual Reports | Document review | 3-5 |
| URSC Technical Documents | Request/search | 2-3 |
| Indian RS conferences (ISRS, ISPRS-India) | Proceedings search | 8-12 |
| NRSC technical bulletins | Direct contact | 3-5 |
| Current Science (ISRO papers) | Database search | 5-8 |
...
| Secondary_Cluster | If applicable |
...
| Accuracy | Reported accuracy metrics |
...
## E. ISRO Cluster Special Protocol
...
site:iirs.gov.in ("forest" OR "hyperspectral" OR "LiDAR")
```
...
- [ ] FSI technical reports on remote sensing
...
| Mission_Fit | HySIS/AVIRIS-NG/Resourcesat/Future | Applicable mission |
| Scalability | Low/Medium/High | Potential for satellite-scale |
| Indian_Relevance | Low/Medium/High | Applicability to Indian forests |
...
| 6 | Paper cards + Initial synthesis | Literature cards draft |
| 7 | Synthesis completion | Lit review draft |

---

## G. Expected Outputs

1. **Comprehensive paper database** with quality scores
2. **Thematic synthesis** per cluster
3. **Gap analysis matrix** supporting research questions
4. **Baseline performance benchmarks** for comparison
5. **ISRO relevance mapping** for all key papers
6. **Recommended citations** for each manuscript section
